  ...
  _createClass(DataPack, [{
    ...
    value: function getSchema(key) {
      ...
      if (typeof key === 'string') {
        ...
        schema = Array.from(this.registry.values()).find(function (schema) {
    ...
    value: function serialize(clientId, items) {
      ...
      items.some(function (item) {
        var schema = _this.getSchema(item.type);
    ...
    value: function deserialize(buffer) {
      ...
      while (offset < buffer.byteLength) {
        var _type = dataView.getUint16(offset);
        var schema = this.getSchema(_type);
